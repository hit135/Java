		...
		int rank = 0;
 		outer:while(true) {
			...
			for(int i = 0; i < p_list.size(); i++) {
				...
				// 말이 들어간 시간 구해서 배열에 담기
				ArrayList<Integer> goalTimes = new ArrayList<>();
				int nowGoal = 0;
				int goalTime = goalTime(p_list.get(i), nowGoal);
				// 도착하면 배열에 담기 // 내 말을 다른 값에 뺌
				if(goalTime != 0) {
				...
				if(allGoal == true) {
					...
					System.out.println(
							"\n===================================== 경마장 ===================================== \n");
					// 시간 담은 배열들 내림차순 정렬
					Collections.sort(goalTimes);
					for(int j = 0; j < goalTimes.size(); j ++) {
						if(goalTimes.get(j) == myTime) {
							rank = j+1;
						}
					}
					break outer;
